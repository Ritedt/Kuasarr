...
from kuasarr.providers.xem_metadata import get_all_season_names
...
def guess_release_title(page_title: str, release_info: ReleaseInfo) -> str:
    """Builds a dotted release title from page title and release info."""
    if not page_title:
        return ""

    # Normalize: replace spaces with dots
    base = re.sub(r'\s+', '.', page_title.strip())

    if release_info.season is not None and release_info.episode_min is not None:
        ep_max = release_info.episode_max
        if ep_max and ep_max != release_info.episode_min:
            ep_str = f"S{release_info.season:02d}E{release_info.episode_min:02d}-E{ep_max:02d}"
        else:
            ep_str = f"S{release_info.season:02d}E{release_info.episode_min:02d}"
        base = f"{base}.{ep_str}"
    elif release_info.season is not None:
        base = f"{base}.S{release_info.season:02d}"

    if release_info.resolution:
        base = f"{base}.{release_info.resolution}"

    return base


def get_at_download_links(shared_state, url, mirror, title, password=None):
