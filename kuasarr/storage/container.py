#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Container Module für kuasarr Storage
...
def get_movies(storage_path):
    """Lädt Filme-Informationen aus dem angegebenen Storage-Pfad"""
    if not storage_path:
        return []
    ...
    try:
        ...
        # Verschiedene Dateierweiterungen für Videos
        ...
        if storage_dir.exists() and storage_dir.is_dir():
            for item in storage_dir.iterdir():
                if item.is_file():
                    ...
                    # Verzeichnisse (möglicherweise Movie-Ordner)
                    movies.append({
                        'title': item.name,
                        'path': str(item),
                        'type': 'directory'
...
def get_tv_shows(storage_path):
    """Lädt TV-Shows-Informationen aus dem angegebenen Storage-Pfad"""
    if not storage_path:
        return []
...
def load_container_info(storage_path):
    """Lädt Container-Informationen aus einer JSON-Datei"""
    try:
        storage_dir = Path(storage_path)
        container_file = storage_dir / 'container_info.json'
...
def get_storage_stats(storage_path):
    """Gibt Statistiken über den Storage-Pfad zurück"""
    try:
        storage_dir = Path(storage_path)
        if not storage_dir.exists():
            return {
